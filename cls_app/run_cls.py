...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [CLS] %(levelname)s: %(message)s"
)
logger = logging.getLogger("cls_app")
...
def process_image(key: str) -> Optional[Dict[str, Any]]:
    """Process an image using the classification endpoint
...
def main() -> None:
    ...
    logger.info(f"Starting classification client, connecting to {REDIS_HOST}:{REDIS_PORT}")
